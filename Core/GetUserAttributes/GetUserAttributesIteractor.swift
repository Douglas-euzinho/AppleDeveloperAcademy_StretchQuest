...
public class GetUserAttributesVersion2: GetUserAttributesIteractor {
    ...
    public func execute(_ result: GetUserAttributesResult) {
        ...
        let midnight = Date.now.midnight

        let flexibility = flexibilitySessions.filter({ $0.start >= midnight }).count
        let posture = postureSessions.filter({ $0.start >= midnight }).count
        let strength = strengthSessions.filter({ $0.start >= midnight }).count
